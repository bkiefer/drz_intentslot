...
adapters_dir: str = "adapters/" + data_type
...
def _annotate_line_slots(line: str) -> dict[str, dict[str, list[str]]]:
    ...
    mask = torch.tensor([encoded_line['attention_mask']]).to(device)
    ...
    for task in tasks:
        ...
        # get predications
        model_result = model(tensor, attention_mask=mask)
